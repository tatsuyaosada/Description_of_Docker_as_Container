...
#### そもそもコンテナって何？
一般的な仮想サーバー、仮想マシン[^1] はベースとなっているホストOS[^2] 上に、更にサブシステムとしてゲストOS[^3] を乗せ、その上でサーバーを走らせると言う手法を取る。当然ながら、そこには両者のOSのプロセスの干渉が発生し、不具合の元となる事も少なくない。分かり易い例としては、XAMPPを常用している人であれば、ホストOSが使う何かしらのアプリケーションとXAMPPのポートが競合し、Apache等が動作しない、なんてケースは日常茶飯事であろう。

一方でコンテナは、ベースOS上にサブシステムとしてゲストOSを載せる必要がない。ベースOSのプロセスに干渉する事が無い。具体的に言うとコンテナを生成するアプリケーション(主としてDocker)の実行環境上にコンテナを生成するので、もちろんDockerはベースOSのプロセス上で動作するが、その上で生成されるコンテナは、ベースOSのプロセスと完全に隔離されている、と考えれば良い。
...
#### 注釈
[^1]: 仮想化ソフトウェアを用い、ホストOS上に更に別のOSを走らせる事。例えばWindows上でLinuxやApacheを動かす等。
[^2]: そのコンピューターを動かす根底となっているOS。ほとんどの人はWindowsをイメージすると分かり良いだろう。
[^3]: 
