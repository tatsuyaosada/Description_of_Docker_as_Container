...
#### ~~話聞いていると、これって別にXAMPPでも良くない？って思うんだけど...~~

~~半分正解。ただ、XAMPPはホストOSのプロセスを確実に侵害する。何かのポートと競合してApacheが起動しない。終了手順を間違えた結果MySqlが起動しないなんて言うのは良くあるトラブルだ。これらは全て、XAMPPがホストOSのプロセスに干渉する事が原因。それが無くなるとしたら...どうだろう。~~

~~また、XAMPPは本番環境に近い形は作れるが、~~
